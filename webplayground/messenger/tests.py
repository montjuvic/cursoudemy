...
class ThreadTestCase(TestCase):
    ...
    def test_add_users_to_thread(self): #LOS TEST SIEMPRE DEBEN EMPEZAR CON "test_"

        self.thread.users.add(self.user1, self.user2)
        self.assertEqual(len(self.thread.users.all()), 2)

#python manage.py test messenger.tests.ThreadTestCase.##NOMBRE DEL TEST##
#python manage.py test messenger.tests.ThreadTestCase.test_add_users_to_thread
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        message1 = Message.objects.create(user=self.user1, content="Muy buenas")
        message2 = Message.objects.create(user=self.user2, content="hola")
        message3 = Message.objects.create(user=self.user3, content="hola")
